...
sub get_group {
    ...
    if ( !exists $self->{ changes }->{ $group } ) {
        $self->{ changes }->{ $group } = CPAN::Changes::Group->new( group_name => $group );
    }
    if ( not blessed $self->{changes}->{$group} ) {
       $self->{ changes }->{ $group } = CPAN::Changes::Group->new( group_name => $group , changes => $self->{changes}->{$group} );
    }
...
sub add_group {
    my $self = shift;
    $self->{ changes }->{ $_ } = CPAN::Changes::Group->new( group_name =>  $_ ) for @_;
}
...
sub delete_empty_groups {
    ...
    $self->delete_group($_->group_name)
...
=head2 get_group( [ $group_name ] )

Returns the internal L<CPAN::Changes::Group> object for the group C<$group_name>.

If C<$group_name> is not specified, the C<default> group C<('')> will be returned.

If C<$group_name> does not exist, a L<CPAN::Changes::Group> object will be created, and returned.
...
as it is instead determined from C<< $group_object->group_name >>
